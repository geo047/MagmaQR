...
void server_close( )  ;
int server_compute_qr_mgpu(  hideprintlog hideorprint  ) ;  // This function internally uses the static CSharedRegion * shrd_server object
...
void PrintUsage(std::string progName) 
{
 ...
 // struct arg_list is defined in CSharedRegion.h
struct arg_list get_qr_args(int argc, char* argv[])
{
	int opt;
 ...
 * 		> qr_server -n 10000 -v 1 -g 3 -m /syevx_<PID_of_client> -s /sem_<PID_of_client> 
 * 		Where:
...
int main(int argc, char* argv[])
{
  ...
  std::string pathstr ; 
	main_args = get_qr_args(argc, argv) ; 
  ...
  if ( main_args.msgChannel == PRINT ) 
	...
	if (main_args.numGPUsWanted > numgpuavail) // check that the number of GPUs requested are available
	{
		if (numgpuavail == 0) 
		{
			ss_string << " MAGMA_EVD_SERVER Error: Number of gpus available is 0. qr_server will now exit" ;
			server_close() ;
      exit(EXIT_FAILURE);
      //error_and_die(ss_string.str());
		}
  ...
  while (exit_server == 0)
  {
     ...
     // std::flush(std::cout) ;
      ...
      {     
          ...
          syevd_info = server_compute_qr_mgpu( main_args.msgChannel ) ;
          if (syevd_info != 0)  
          {
            sem_post(shrd_server->_sem_id) ;
            ss_string << " MAGMA_EVD_SERVER Error: Error from: " << argv[0] ;
            ss_string << " info != 0 returned from server_compute_qr_mgpu() " << std::endl  ;
...
int server_compute_qr_mgpu( hideprintlog hideorprint )
{
  ...
  double *work, *rvectors_ptr, *rvalues_ptr, *T  ;
  ...
  if (shrd_server == NULL)
  {
    std::cerr << " MAGMA_QR_SERVER Error: server_compute_qr_mgpu(): shrd_server object is NULL" << std::endl ;
    return(-1) ;
  }
  ...
  magma_dmalloc_cpu( &hA,     lda*n  );
  magma_dmalloc_cpu( &hR,     lda*n  );
...
//  magma_dgetmatrix(  n, n, dA,  lda,  rvectors_ptr , ldda, queue );
...
// double *hR;
//  magma_dmalloc_cpu(&hR, lda*n);
//  lapackf77_dlacpy( MagmaFullStr, &n, &n, rvectors_ptr , &n, hR, &n );
 // magma_dorgqr_m( n, n, n, hR , n , tau, dT, nb, &info );
//     std::cout << " Q "<< std::endl;
//     magma_dprint(5,5, hR , n );
...
/*
  ...
  magma_dmalloc_cpu(&tau, n);
  magma_dmalloc_cpu(&T, nb*n);

  if (shrd_server->_numgpus == 1) {
     magma_dgeqrf( n, n, rvectors_ptr  , n   , tau, work, lwork, &info );
     std::cout << " h_A afer dgeqrf"  << std::endl;
     magma_dprint(5,5, rvectors_ptr, n);
     ...
     magma_dgeqrf_m(shrd_server->_numgpus  , n, n, rvectors_ptr  , n   , tau, work, lwork, &info );
     std::cout << " h_A afer dgeqrf"  << std::endl;
     magma_dprint(5,5, rvectors_ptr, n);
     magma_dorgqr_m(n, n, n, rvectors_ptr, n, tau, T, nb, &info );	
     std::cout << " Q "<< std::endl;
     magma_dprint(5,5, rvectors_ptr, n);
  }
  ...
  return (0) ;
   ...
   std::vector<cl::Platform> platforms;
    ...
    if (platnum == -1)
    {
      ...
      std::cerr << " MAGMA_EVD_SERVER Error: an available platform (as seen above)in the <R_LIB>/MagmaQR/src/make.inc file " << std::endl ;
    }
