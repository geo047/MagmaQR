...
#' @export
#' @description 
#' Creates solve_server executable using a call to 'make' and the makefile/make.inc information present in the <package root>/src directory.
...
{ 
  # R.home() system.file(package="MagmaQR") Sys.info()['sysname']
  package_path <- path.package("MagmaQR")
...
RunServer <- function( environmentSetup="", numGPUsWanted=0,  matrixMaxDimension=0, memName="", semName="", print=0)
{
  ...
  if (matrixMaxDimension <= 0 ) {
    stop("MAGMA_EVD_CLIENT Error: MagmaQR::RunServer(): Input matrix dimension should specify the maximum size of a matrix to undergo eigenvalue decomposition")
  }
  
  if (print > 1) {
    message("MAGMA_EVD_CLIENT Info: MagmaQR::RunServer(): Only print to screen is currently available")
    print <- 1 ;
  }
...
.onAttach <- function(libname, pkgname) { 
  ...
  # packagepathstring <- libname ;# path.package ("MagmaQR");  
  serverstring = paste0(libname,"/MagmaQR/bin/solve_server.exe")
 # file.exists(c("/home/bow355/R/library/MagmaQR/bin/solve_server"))
  if (!file.exists(serverstring)) {
     ...
     envfile <- paste0(libname,"/extdata/environmentSetup.rds")
      ...
      if (envexist==1) {
        ...
        packageStartupMessage("\t...the MagmaQR server executable was not found so we have attempted to rebuild")
        if (file.exists(serverstring)) { 
